...
def watermark(input_directory, output_directory):
    ...
    data.to_json(
        output_directory / "watermarked-text.jsonl", lines=True, orient="records"
    )
...
#         model_id, device_map="auto", **model_args
#     )
...
#     output_directory.mkdir(exist_ok=True, parents=True)
#     with open(
#         str(output_directory / "watermarked-text.jsonl"), "w", encoding="utf-8"
#     ) as outfile:
...
#     output_directory.mkdir(exist_ok=True, parents=True)
#     with open(
#         str(output_directory / "detected-text.jsonl"), "w", encoding="utf-8"
#     ) as outfile:
#         for _, entry in data.iterrows():
#             json.dump(entry.to_dict(), outfile, ensure_ascii=False)
#             outfile.write("\n")
...
main.command()(detect)
# main.command()(watermark_torch)
# main.command()(detect_torch)
